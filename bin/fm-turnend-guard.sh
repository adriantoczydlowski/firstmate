...
#      failopen_condition_verified below. The budget charges each event epoch
#      once, and it also charges every re-block against an epoch the auto-arm
#      never advanced past the previous re-block (budget_account_current_epoch
#      owns that rule), so an inert hook that leaves the ledger frozen cannot
#      hold the guard in an unbounded re-block loop below that override.
set -u
