...
# suspended one, does not block. Such a holder is converged onto the acquiring
# session's own pid, so repeated runs are idempotent, and any live holder it
# converged onto, took over from, or could not identify is named on stdout
# rather than being silent. The two in-place proofs above - ancestry membership
# and the trusted session id, together fm_session_lock_owned_in_place - are the
# exception: they name a pid this session still reaches, so they are confirmed
...
confirm_own_lock() {  # <recorded-pid>
  ...
  if [ "$recorded" = "$me" ] || fm_session_lock_owned_in_place "$STATE"; then
    publish_lock_session_or_die
    commit_lock_session
    release_claim_lock
...
if [ -f "$LOCK" ] && [ ! -L "$LOCK" ]; then
  ...
  if [ "$old" = "$me" ] || fm_session_lock_owned_in_place "$STATE"; then
    confirm_own_lock "$old"
    old=$(cat "$LOCK" 2>/dev/null || true)
  fi
  if fm_session_lock_holder_competes "$old" "$STATE"; then
...
if [ -e "$LOCK" ] || [ -L "$LOCK" ]; then
  ...
  if [ "$old" != "$me" ]; then
    if fm_session_lock_owned_in_place "$STATE"; then
      confirm_own_lock "$old"
      old=$(cat "$LOCK" 2>/dev/null || true)
    fi
  fi
