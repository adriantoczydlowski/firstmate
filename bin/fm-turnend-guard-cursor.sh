...
# Only the lock-owning session may arm or wake. A prior session that died or was
# suspended leaving its numeric harness pid behind is the recoverable case,
# delegated to bin/fm-lock.sh so acquisition keeps its single owner.
