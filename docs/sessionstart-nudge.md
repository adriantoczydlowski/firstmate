...
That shared walk can extend past a claude-named match to a still-more-ancestral one, and the ownership decision it feeds additionally accepts a lock held by a co-located session that provably started this one or was provably started by it, or one recorded under this same trusted Claude session id.
