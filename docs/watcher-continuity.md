...
That third holder is recognized from a launch marker the harness writes into the processes it starts, read in whichever direction of that pair recorded it, which is verified for Claude only, so on codex, opencode, pi, pi-signed, grok, kimi, and cursor this boundary is still decided by process ancestry alone.
