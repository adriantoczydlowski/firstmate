...
A numeric session-lock owner that fails the shared `fm_session_lock_holder_competes` predicate is reclaimed through `bin/fm-lock.sh` before auto-arm state changes, while a competing owner, absent lock, or malformed lock keeps the hook inert.
Whether the session already owns that lock is the shared `fm_session_lock_owned_by_self` verdict in `bin/fm-session-lock-lib.sh`, which accepts a recorded pid inside the current harness ancestry, a co-located holder the session-cohort proof claims, or a live lock recorded under this same trusted Claude session id, so a background session keeps arming after its transient helper chain is recycled.
That predicate is the single owner of the session-lock recovery boundary in `bin/fm-session-lock-lib.sh`, and four holders do not compete: a pid that is gone, a live pid the identity rules decline to type as a harness, a durably stopped harness, and a live harness co-located with this session that either provably started it or was provably started by it.
Only the first of those is reclaimed silently, because a holder that no longer exists is the ordinary path every session start takes.
The second reclaims a process that was still present when the predicate classified it, so it is announced rather than silent, and the condition it reports is general: some live process occupies the recorded pid and the identity rules cannot type it as a harness.
Most often that means the recorded pid no longer belongs to any harness at all, because a session died without releasing `state/.lock` and the pid was later reused by an unrelated process, which is the benign and expected case rather than a fault to chase.
Read the line as a statement about the recorded pid rather than as a diagnosis of whatever now occupies it, and treat a lock an older firstmate wrote for a still-live session whose install shape the current identity rules do not identify as one named instance of the same condition, which [`verification/runtime-backends.md`](verification/runtime-backends.md#session-lock-identity-and-the-suspended-holder) owns as a bounded adoption-window limit.
That general condition is exactly why this case reclaims rather than refuses: refusing on a live but unidentified pid would let ordinary pid reuse lock a home read-only, which is the defect this boundary exists to remove.
The co-located holder is recognized from a launch marker the harness writes into the processes it starts, read in whichever direction of that pair recorded it, and consulted only when the asking session and the holder are both the harness that marker was verified for, which is Claude alone, so on codex, opencode, pi, pi-signed, grok, kimi, and cursor this boundary is still decided by process ancestry alone.
Both sides of that pair are named from executable identity alone, never from an argument list, because a launch marker proves a launch relationship rather than identity and a different harness started from inside a session inherits it truthfully.
The pair must also be ORDERED, because a marker is an environment snapshot matched against the holder by numeric equality and so keeps matching once that pid has been recycled onto an unrelated session of the same harness.
The pid a marker names must therefore not have started after the process that carried that marker, which a recycled occupant cannot satisfy, and an unreadable start time on either side refuses the relationship and falls back to the ancestry verdict.
Do not read the co-located holder as cross-harness behavior: `FM_SESSION_LAUNCH_MARKERS` in that file owns the limit and why it is safe, and [`verification/runtime-backends.md`](verification/runtime-backends.md#session-lock-identity-and-the-suspended-holder) owns the per-harness evidence.
The stopped case is a deliberate decision rather than pure liveness, because `kill -0` succeeds on a stopped process and treating it as still holding turns one suspended session into a permanent lockout of the whole home.
It is confirmed over several samples so a momentary stop is not read as a suspended session, and it is made visible rather than surprising: `bin/fm-lock.sh status` reports a suspended holder as reclaimable, and acquisition names any live holder it yielded past, distinguishing a genuine takeover from a suspended session, one session converging its own lock onto its own pid, and a reclaim from a live holder it could not identify.
[`turnend-guard.md`](turnend-guard.md#guard-predicates) owns the Claude guard's behavior when that live owner is neither inside the current session's harness ancestry nor recorded under its trusted session id.
