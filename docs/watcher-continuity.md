...
That predicate is the single owner of the session-lock recovery boundary in `bin/fm-session-lock-lib.sh`, and three holders do not compete: a dead or non-harness pid, a durably stopped harness, and a live harness that provably started this session and is co-located with it.
That third holder is recognized from a launch marker the harness writes into the processes it starts, which is verified for Claude only, so on codex, opencode, pi, pi-signed, grok, kimi, and cursor this boundary is still decided by process ancestry alone.
