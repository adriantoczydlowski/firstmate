...
It is confirmed over several samples so a momentary stop is not read as a suspended session, and it is made visible rather than surprising: `bin/fm-lock.sh status` reports a suspended holder as reclaimable, and acquisition names any takeover from a holder that is still alive.
