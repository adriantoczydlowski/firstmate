...
That predicate is the single owner of the session-lock recovery boundary in `bin/fm-session-lock-lib.sh`, and three holders do not compete: a dead or non-harness pid, a durably stopped harness, and a live harness co-located with this session that either provably started it or was provably started by it.
That third holder is recognized from a launch marker the harness writes into the processes it starts, read in whichever direction of that pair recorded it, and consulted only when the asking session and the holder are both the harness that marker was verified for, which is Claude alone, so on codex, opencode, pi, pi-signed, grok, kimi, and cursor this boundary is still decided by process ancestry alone.
...
Do not read the third holder as cross-harness behavior: `FM_SESSION_LAUNCH_MARKERS` in that file owns the limit and why it is safe, and [`verification/runtime-backends.md`](verification/runtime-backends.md#session-lock-identity-and-the-suspended-holder) owns the per-harness evidence.
