...
A lock outside that ancestry is decided by two further proofs, each sufficient on its own.
The session-cohort proof accepts a co-located holder this session provably started or was provably started by, and its per-harness evidence lives in [`runtime-backends.md`](runtime-backends.md#session-lock-identity-and-the-suspended-holder).
A background Claude session whose transient helper chain is recycled loses contiguity while its recorded owner stays alive, so the library also accepts a trusted same-session id: `CLAUDE_CODE_SESSION_ID` counts only when `CLAUDE_PID` is a Claude-shaped member of the current run, it must equal the id `bin/fm-lock.sh` recorded in `state/.lock-session`, and the recorded pid must still be a live harness, while every weaker combination (no id, no sidecar, an untrusted id, a different id, a dead recorded pid) leaves the ancestry verdict unchanged.
...
The two interact in one direction: when the lock records a session id and the id proof still refuses this process, that refusal is final and the cohort proof is not consulted after it, because the launch marker it would read is a second field of the same environment block the refused id came from.
A lock with no recorded id has no identity to weigh and leaves the cohort proof free.
...
The same suite was re-run on 2026-09-20 on Linux 6.18 (WSL2) with bash 5.2.21, where a `systemd --user` child subreaper adopts orphans instead of init: `tests/fm-session-lock-ancestry.test.sh` exits 0 with all 39 cases passing.
The recycled-chain fixture reads the reparenting itself rather than a fixed parent pid for that reason, the same signal the sibling detached-tree fixture already used.
