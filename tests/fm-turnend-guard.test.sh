...
# still provably run out, and the frozen ledger must survive every stop
# untouched; but the stop does NOT keep blocking forever after that, because a
# ledger whose last entry is older than one event epoch is itself proof that the
# auto-arm is no longer establishing supervision, and that is the second way
# into the same bounded attended valve. This case pins the cause the notice
# reports: an auto-arm on record that stopped running is not an auto-arm that
# never ran, and neither is an exhausted retry.
test_hook_claude_mode_frozen_unverified_epoch_spends_budget_then_reaches_the_stale_fail_open() {
  local dir out status i count frozen
  dir=$(make_primary_dir "$TMP_ROOT/hook-claude-frozen-unverified")
  : > "$dir/state/task1.meta"
  frozen='epoch=7 owner_pid=999 outcome=clean updated_at=1'
  printf '%s\n' "$frozen" > "$dir/state/.claude-autoarm-epoch"
  touch -t 202001010000 "$dir/state/.claude-autoarm-epoch"
  for i in 1 2 3; do
    out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false); status=$?
    expect_code 2 "$status" "frozen unverified stop $i must keep blocking inside the budget"
    assert_not_contains "$out" 'systemMessage' "the valve opened at stop $i, before the budget was spent"
    assert_absent "$dir/state/.claude-autoarm-failure-alarmed" "stop $i consumed the attended alarm early"
    [ "$(sed -n '1p' "$dir/state/.claude-autoarm-epoch")" = "$frozen" ] \
      || fail "the guard rewrote the frozen ledger at stop $i"
  done

  out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false); status=$?
  expect_code 0 "$status" "a frozen ledger with an exhausted budget must reach the bounded attended fail-open"
  count=$(sed -n '2s/^count=//p' "$dir/state/.turnend-claude-blocks")
  [ "$count" -gt 3 ] 2>/dev/null || fail "the block budget must run out against a frozen epoch, but the recorded count is ${count:-absent}"
  assert_contains "$out" 'FIRSTMATE SUPERVISION IS GENUINELY DOWN' "the stale-ledger fail-open was not unmistakable"
  assert_contains "$out" 'last recorded an attempt' "the stale-ledger fail-open did not name the real condition"
  assert_not_contains "$out" 'never claimed this home at all' "an auto-arm with an entry on record was reported as one that never ran"
  assert_not_contains "$out" 'exhausted its bounded retries' "an unverified frozen epoch was reported as an exhausted retry"
  assert_present "$dir/state/.claude-autoarm-failure-alarmed" "the stale-ledger fail-open did not consume the episode alarm"
  [ "$(sed -n '1p' "$dir/state/.claude-autoarm-epoch")" = "$frozen" ] \
    || fail "the guard rewrote the frozen ledger at the fail-open stop"

  out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false); status=$?
  expect_code 2 "$status" "a consumed attended alarm must make later unhealthy stops block again"
  assert_not_contains "$out" 'FIRSTMATE SUPERVISION IS GENUINELY DOWN' "the stale-ledger alarm repeated in one episode"
  pass "fm-turnend-guard --claude: a frozen unverified epoch spends the budget and then reaches one bounded stale fail-open"
}
