...
wait_for_background_file() {  # <path> <what>
  local i=0
...
fire_phase() {  # <dir> <n> <hook-environment-script>
  ...
  wait_for_background_file "$dir/state/phase-$n/hook.rc" "phase $n"
  local i=0
...
test_e2e_background_session_keeps_its_lock_across_a_recycled_chain() {
  local dir frontend daemon ptyhost ptyhost_parent spare i
  ...
  wait_for_background_file "$dir/state/frontend-lock.rc" "the front-end's lock result"
  wait_for_background_file "$dir/state/spare-pid" "the bg-spare"
  ...
  # it, and the front-end that holds the lock stays alive. The property this
  # phase needs is the reparenting itself, not the pid it lands on: an orphan
  # reaches init only where no child subreaper is installed, and a systemd user
  # manager adopts it instead. The sibling fixture above reads the same signal
  # the same way.
  kill -TERM "$daemon"
  i=0
  ...
  [ -n "$ptyhost_parent" ] && [ "$ptyhost_parent" != "$daemon" ] \
    || fail "the pty-host was not reparented away from the daemon after it ended (parent: ${ptyhost_parent:-none})"
...
SH
  ...
  wait_for_background_file "$dir/state/acquire.rc" "the initial lock acquisition"
  ...
  wait_for_background_file "$dir/state/holder-pid" "the claim-lock holder pid"
  holder_pid=$(tr -d '[:space:]' < "$dir/state/holder-pid")
  BG_FIXTURE_PIDS+=("$holder_pid")
  wait_for_background_file "$dir/state/confirm-pid" "the same-session confirmation pid"
  confirm_pid=$(tr -d '[:space:]' < "$dir/state/confirm-pid")
  BG_FIXTURE_PIDS+=("$confirm_pid")
  wait_for_background_file "$dir/state/confirm.rc" "the contended confirmation result"
...
SH
  ...
  expect_code 0 "$(tr -d '[:space:]' < "$dir/state/acquire.rc")" \
    "the session could not acquire its lock: $(cat "$dir/state/acquire.out")"
  wait_for_background_file "$dir/state/other-pid" "the other live harness pid"
...
trap 'cohort_cleanup; reap_background_fixture; fm_test_cleanup' EXIT
trap 'cohort_cleanup; reap_background_fixture; fm_test_cleanup; exit 130' INT
trap 'cohort_cleanup; reap_background_fixture; fm_test_cleanup; exit 143' TERM
